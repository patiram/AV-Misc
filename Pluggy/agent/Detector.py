...
class Detector(threading.Thread):
    ...
    def __init__(self, conf, plugin):
        ...
        self._sensorID = None
        if "ctx" in self._conf.hitems("plugin-defaults"):
            self._correlationContextID = self._conf.get("plugin-defaults", "ctx")
...
#        self.patternUTClocalized = re.compile('(?P<year>\d+)[\s-](?P<month>\d+)[\s-](?P<day>\d+)\s+(?P<hour>\d+):(?P<minute>\d+):(?P<second>\d+)(?P<tzone_symbol>[-|+])(?P<tzone_hour>\d{2}):(?P<tzone_min>\d{2})')
    ...
    def _plugin_defaults(self, event):
        ...
        if self._correlationContextID is not None:
            event['ctx'] = self._correlationContextID
        ...
        # 6) sensor,source ip and dest != localhost
            if event["sensor"] in ('127.0.0.1', '127.0.1.1') and not self.override_sensor:
                event["sensor"] = default_sensor
    ...
    def send_message(self, event):
        ...
        Output.event(event, self._plugin.get('config', 'send_policy'))
