...
def resolv(plugin, host):
    ...
    try:
        ...
        print "Leaving without host 1 " + host
	return host

    print "Leaving with host 1"
    return addr


def resolv_ip(plugin, addr):
...
def md5sum(plugin, datastring):
    return md5.new(datastring).hexdigest();


def snort_id(plugin, id):
    return str(1000 + int(id))


def normalize_protocol(plugin, protocol):
...
def normalize_date_american(plugin, string_date):
...
def normalize_date(plugin, string, american_format=False):
    ...
    if american_format:
        ...
        if result:
            ...
            try_other = False
	    plugin.dateRegex = pattern

    elif hasattr(plugin, 'dateRegex'):
         result = plugin.dateRegex.search(string)
	 if result is not None:
            dict = result.groupdict()
            if not check_date(dict):
                   dict.clear()
		   try_other = True 
...
def upper(plugin, string):
    return string.upper()

def sanitize(plugin, data):
    return data.replace("\n", "\r")



def hextoint(plugin, string):
    try:
...
def intrushield_sid(plugin, mcafee_sid, mcafee_name):
...
def netscreen_idp_sid(plugin, message):
...
def iss_siteprotector_sid(plugin, message):
...
def translate_wsaea_IDs(plugin, string):
    string_translated = ''
