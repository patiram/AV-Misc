...
class NmapManager:
    ...
    def process(self, data, base_response):
        ...
        if action == "nmap_scan":
            ...
            scan_type = Utils.get_var("type=\"(ping|0|full|1|fast|2)\"" , data)
...
class DoNmap (threading.Thread):
    ...
    def set_scan_type(self, scan_type):
        ...
        elif scan_type == 1 or scan_type == "full":
            self.__scan_type = "full"

        elif scan_type == 2 or scan_type == "fast":
    ...
    def run(self):
        ...
        while True:
            ...
            if self.__scan_type == "ping":
                ...
                cmd = "%s -A -sS -O -sV -n %s -oX %s --no-stylesheet" % (self.__nmap_bin_path, self.__target, nmap_report)
            elif self.__scan_type == "fast":
                cmd = "%s -F -A -sS -O -sV -n %s -oX %s --no-stylesheet" % (self.__nmap_bin_path, self.__target, nmap_report)
            else:
                self.__status = -1
